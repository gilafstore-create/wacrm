...
function IntegrationCredentials({ intg, onRefresh }: { intg: Integration; onRefresh: () => void }) {
  ...
  const generateToken = async () => {
    if (!confirm("Generate a new connection token?\n\nThe old token will stop working immediately. Paste the new token in GilafStore → Admin → WACRM Connection.")) return;
    setRegenToken(true);
    setNewToken(null);
    try {
      const r = await fetch("/api/integrations", {
      ...
      const d = await r.json();
      if (d.connection_token) {
        setNewToken(d.connection_token);
        navigator.clipboard.writeText(d.connection_token);
        setCopied("new_token");
        setTimeout(() => setCopied(null), 3000);
        onRefresh();
      } else {
  ...
  const tokenStatus = intg.connection_token === "(used)" ? "Used" : intg.connection_token ? "Active" : "None";
  ...
  return (
    <GlassCard className="p-5">
      ...
      <div className="mt-4 pt-4 border-t border-white/5">
        <div className="flex items-center justify-between mb-2">
          <div>
            <p className="text-xs font-semibold text-slate-300">Connection Token</p>
            <p className="text-[11px] text-slate-500 mt-0.5">Paste in GilafStore → Admin → WACRM Connection → Quick Connect</p>
          </div>
          <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-medium ${
            tokenStatus === "Used"   ? "border-amber-500/30 bg-amber-500/10 text-amber-400"
            : tokenStatus === "Active" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-400"
            : "border-slate-700 bg-slate-800 text-slate-500"
          }`}>
            <span className={`h-1.5 w-1.5 rounded-full ${tokenStatus === "Used" ? "bg-amber-400" : tokenStatus === "Active" ? "bg-emerald-400" : "bg-slate-500"}`} />
            {tokenStatus}
          </span>
        </div>
        ...
        {newToken ? (
          <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-3 mb-3">
            <p className="text-[11px] font-medium text-emerald-400 mb-1.5">New token — copy now, auto-copied to clipboard!</p>
            <div className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-900 px-3 py-2">
              <code className="flex-1 break-all font-mono text-xs text-emerald-300 select-all">{newToken}</code>
              <button onClick={() => handleCopy(newToken, "new_token")} className="shrink-0 text-slate-400 hover:text-white transition-colors" title="Copy token">
                {copied === "new_token" ? <Check className="h-4 w-4 text-emerald-400" /> : <Copy className="h-4 w-4" />}
              </button>
            </div>
          </div>
        ) : (
          <div className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-900/60 px-3 py-2 mb-3">
            <code className="flex-1 truncate font-mono text-xs text-slate-400">
              {intg.connection_token === "(used)"
                ? "gs_connect_•••••••• (already activated — generate new to reconnect)"
                : intg.connection_token
                ? intg.connection_token
                : "No token yet — click Generate below"}
            </code>
            {intg.connection_token && intg.connection_token !== "(used)" && (
              <button onClick={() => handleCopy(intg.connection_token ?? "", "token")} className={`shrink-0 rounded p-1 transition-colors ${copied === "token" ? "text-emerald-400" : "text-slate-500 hover:text-white hover:bg-slate-700"}`} title="Copy token">
        ...
        <button
          ...
          disabled={regenToken}
          className="flex items-center gap-2 rounded-lg border border-violet-500/40 bg-violet-500/10 px-4 py-2 text-xs font-medium text-violet-300 hover:bg-violet-500/20 transition-colors disabled:opacity-50"
        >
          {regenToken ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RotateCcw className="h-3.5 w-3.5" />}
          {regenToken ? "Generating…" : "Generate New Token"}
        </button>
      </div>
